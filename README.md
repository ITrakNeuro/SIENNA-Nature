...
This repository offers the pre-trained weights of SIENNA, specifically trained on clinical data. The patient_wise_analysis.py script sequentially retrieves trimmed MRI scans from designated data paths, applies "Pixel Redistribution Enhancement, Masking, Optimization" (PREMO) for equalization, and makes predictions using the SIENNA pre-trained architecture. The patient-specific results are then presented in using quantifiable metrics, such as Accuracy and F1-Score.
